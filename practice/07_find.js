...
const foundPost = posts.find((value) => {  // find는 객체가 배열이기 때문에만 가능
...
// 5. 문자열로 시작되는 것을 검색
const document = [
    { id: "DOC-2024_01", title: "연간 보고서" },
    { id: "MEMO_2024_05", title: "회의록"},
    { id: "DOC_2024_02", title: "분기 실적" },
];
// "DOC"로 id가 시작하는 첫 번째 문서 찾기 => 전두사 (prefix)
...
// 파일명이 ".png"로 끝나는 첫번 째 이미지 찾기  => 접미사(suffix)
const foundFiles = files.find ((value) => {
    return vlaue.name.endsWith(".png");            // 접미사(suffix)로 끝나면 true, 아니면 false
});
...
// "전자" 제품 중에서 제고가 5개 이상이거나 이름이 2글자 초과하는 첫번째 상품 찾기
