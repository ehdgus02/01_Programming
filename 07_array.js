...
arr.push("cola", "water");   // push() 메소드는, 배열의 마지막에 값을 추가할 수 있음. 매개변수 1개 이상 쓸 수 있음
...
Array.isArray(arr)      // 집어넣은 변수(값)가 배열인지 아닌지 => ture, false
...
b.forEach(
    () => {         // 매개변수는 현재 순회하는 "그" 요소를 받아주어야 함수에서 처리가 가능함
        // 실제 실행되어야 하는 함수 몸통
        console.log();
    }
)

// add라는 함수를 실행시킬 때, 첫 번째 자리에 들어오는 값을 x라고 내가 부르겠다. 두번째 자리에 들어오는 값을 y라고 내가 부르겠다.
function add(x, y) {
    return x + y;
}
...
// i  = i  => 부모가 갖고 있는 sum = 1500 => 1500 + prices[1] = 1500 + 3000 => 부모가 갖고 있는 sum을 4500으로 바꿈
// i = 2 => 부모가 갖고 있는 sum = 4500 => 4500 + prices[2] = 4500 + 500 => 부모가 갖고 있는 sum은 5000으로 바꿈
